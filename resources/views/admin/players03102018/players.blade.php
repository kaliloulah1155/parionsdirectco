                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table id="zero_config" class="table table-striped table-bordered">
                                ...
                                <tbody>
                                @for ($i = 0; $i < $cmpttotaldata; $i++)
                                    <tr>
                                        <td>{{  $results->data->list[$i]->firstname }}</td>
                                        <td>{{  $results->data->list[$i]->lastname }}</td>
                                        <td>{{  $results->data->list[$i]->email }}</td>
                                        <td>{{  $results->data->list[$i]->msisdn }}</td>
                                        <td>{{  (new DateTime($results->data->list[$i]->created_at))->format('d/m/Y à h\h:i\m\n ') }}</td>
                                        <td>{{  (new DateTime( $results->data->list[$i]->confirmed_at))->format('d/m/Y à h\h:i\m\n ') }}</td>
                                        <td>{{  (new DateTime( $results->data->list[$i]->last_connection_date))->format('d/m/Y à h\h:i\m\n ') }}</td>
                                        <td style="text-align: center">
                                            <a  href="{{ URL::to('/admin/players_profil/'.$results->data->list[$i]->id) }}"  class="btn btn-cyan btn-sm  mdi mdi-account-star-variant" title="Détails"></a>
                                        </td>
                                    </tr>
                                @endfor
                                </tbody>
